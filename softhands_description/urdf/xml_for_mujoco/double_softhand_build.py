...
def build_hand(xml_path, operations=ALL_OPERATIONS, **config):
...
def main():
    ...
    xml_files = [
        "/home/esguerri/SoftHand/src/SoftHand-Plugin/softhands_description/urdf/xml_for_mujoco/right_softhand_v1_2_research.xml",
        "/home/esguerri/SoftHand/src/SoftHand-Plugin/softhands_description/urdf/xml_for_mujoco/left_softhand_v1_2_research.xml"
    ]
    hands_mjcf = []
    for xml_file in xml_files:
        hand_mjcf = build_hand(xml_path=xml_file)
        hands_mjcf.append(hand_mjcf)
        print('hands_mjcf: ', hands_mjcf)
        print(f"Mano costruita da {xml_file}")
        model_name = os.path.basename(xml_file).split('.')[0]  
        print(model_name) 
        destination = "export/"
        xml = re.sub("(-\w+)(?=\.stl)", "", hand_mjcf.to_xml_string())
        xml = re.sub("<compiler", f"<compiler meshdir=\"{hand_mjcf.compiler.meshdir}\"", xml)
        f_name = destination + model_name + '_edited.xml'
        with open(f_name, 'w') as file:
         file.write(xml)

    models = []
    for hand_mjcf in hands_mjcf:
        model = mujoco.MjModel.from_xml_string(hand_mjcf.to_xml_string(), hand_mjcf.get_assets())
        models.append(model)
        print('models: ', models)
    
    for model in models:
     thread = threading.Thread(target=launch_viewer, args=(model,))
     thread.start()
